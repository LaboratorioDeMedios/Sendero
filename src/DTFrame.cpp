...
DTFrame::DTFrame(int iPixelQuantity)
{
    ...
    this->pixelArray = new cl_float[iPixelQuantity * 4];
...
DTFrame::DTFrame(long iTimeStamp, vector<DTPixel*>* iVectorPixels,float* iPixelArray, int iPixelQuantity ,  unsigned short iSequence)
{
	this->clientID=0;
	this->clientName="Server";
    ...
    this->pixelArray=(cl_float*)iPixelArray;
    this->pixelQuantity=iPixelQuantity;
}
...
DTFrame::DTFrame(string sourceXML)
{
	...
	//if( XML.bDocLoaded ){
        ...
        this->pixelArray = new cl_float[numPixelTags * 4];
		if (myFrame){
            ...
            if (protocolVersion==1){
                ...
                if (myPixel){
					...
					for( myPixel; myPixel; myPixel=myPixel->NextSiblingElement())
					{
                        ...
                        //we pass 0 as ledType, since is just dummy information here
						DTPixel* newPixel = new DTPixel(id,r,g,b,a,positionPixel.x,positionPixel.y,positionPixel.z,front,up,linkedMesh,0);
                ...
                for (int i=0; i<parserPixels.size(); i++){
                    ...
                    //we pass 0 as ledType since is just dummy information here
                    DTPixel* newPixel = new DTPixel(parsedId,parsedR,parsedG,parsedB,parsedA,0,0,0,dummyFront,dummyUp,"dummy",0);
                    
                    newVector->push_back(newPixel);
                    
                    this->pixelArray[(parsedId*4)] = (cl_float)parsedR;
                    this->pixelArray[(parsedId*4)+1] = (cl_float)parsedG;
                    this->pixelArray[(parsedId*4)+2] = (cl_float)parsedB;
                    this->pixelArray[(parsedId*4)+3] = (cl_float)parsedA;
...
void DTFrame::addPixel(DTPixel *newPixel){
    ...
    int pixelID = newPixel->getId();
    if (pixelID<this->pixelQuantity){
        this->pixelArray[(pixelID*4)]= (cl_float)newPixel->getR();
        this->pixelArray[(pixelID*4)+1]= (cl_float)newPixel->getG();
        this->pixelArray[(pixelID*4)+2]= (cl_float)newPixel->getB();
        this->pixelArray[(pixelID*4)+3]= (cl_float)newPixel->getA();
    }
}
...
cl_float* DTFrame::getPixelsArray()
{
...
packet_header_t DTFrame::getBinaryPacketHeader(unsigned short sequence){
    packet_header_t result;
    memcpy(&result.id, "Sendero1", 8); 
    unsigned short version = 1;
    unsigned short crcCheck = 0;
    result.crc = crcCheck;
    result.ver=short_get_low_byte(version);
    result.sequenceHi = short_get_high_byte(sequence);
    result.sequence = short_get_low_byte(sequence);
...
int DTFrame::getPixelQuantity(){
    
    return this->pixelQuantity;
    
}

uint8_t * DTFrame::getRawFrameData(){
    
    uint8_t * res = new uint8_t[this->pixelQuantity*3];
    
    vector <DTPixel*>::iterator it= this->pixels->begin();
    int dataIndex=0;
    while(it!=this->pixels->end()){
        DTPixel* px = *it;
        //store values for pixel
        unsigned short R = (unsigned short) px->getR();
        unsigned short G = (unsigned short) px->getG();
        unsigned short B = (unsigned short) px->getB();
        res[dataIndex] = short_get_low_byte(R);
        dataIndex++;
        res[dataIndex] = short_get_low_byte(G);
        dataIndex++;
        res[dataIndex] = short_get_low_byte(B);
        dataIndex++;
        
        it++;
    }
        
    //res must be deleted by caller.
    return res;
